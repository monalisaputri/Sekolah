package daoimp;
// bikin menu lagi buat cari by id sm nama siswa :(
...
public class SiswaIPADAOImpl implements SiswaIPADAO{
	...
	public void getSiswaIPAById(int id) {
		...
		try {
			db.connect();
            String query = "SELECT * FROM siswaIPA WHERE id = '"+id+"'";
	...
	public void getSiswaIPAByNama(String nama) {
		...
		try {
			db.connect();
            String query = "SELECT * FROM siswaIPA WHERE nama = '"+nama+"'";
            ResultSet rs = db.readData(query);
	...
	public void saveSiswaIPA(SiswaIPA siswaIPA) {
		...
		try {
			db.connect();
			query = "INSERT INTO siswaIPA (id, nama, email, lama_tunggak, bayar_spp,beasiswa)"
	...
	public void updateSiswaIPA(SiswaIPA siswaIPA) {
		...
		try {
			...
			query = "UPDATE siswaIPA SET nama='"+siswaIPA.getNama()+"',"
					...
					+ " beasiswa="+siswaIPA.getBeasiswa()+" WHERE id='"+siswaIPA.getId()+"'";
			db.executeQuery(query);
